...
impl LuaEngine {
    ...
    fn register_api(&self, context: &CommandContext) -> Result<(), LuaError> {
        ...
        api::core::register(&self.lua, &babu)?;
        api::audio::register(&self.lua, &babu)?;
        api::context::register(&self.lua, &babu, context)?;

        // sandbox-controlled APIs
        if self.sandbox.allows_http() {
            api::http::register(&self.lua, &babu)?;
        }

        if self.sandbox.allows_state() {
            api::state::register(&self.lua, &babu, &context.command_path)?;
        }

        if self.sandbox.allows_fs() {
            api::fs::register(&self.lua, &babu, &context.command_path, self.sandbox)?;
        }

        api::system::register(&self.lua, &babu, self.sandbox)?;
