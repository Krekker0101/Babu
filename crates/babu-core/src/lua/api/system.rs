...
pub fn register(lua: &Lua, babu: &Table, sandbox: SandboxLevel) -> mlua::Result<()> {
    let system = lua.create_table()?;

    // babu.system.open(url_or_path) - always available
    ...
    // babu.system.exec(cmd, args?) - only in full sandbox
    ...
    // babu.system.notify(title, message) - always available
    ...
    // babu.system.clipboard - subtable
    let clipboard = lua.create_table()?;

    // babu.system.clipboard.get() - always available
    ...
    // babu.system.clipboard.set(text) - only in full sandbox
    ...
    // babu.system.env(name) - get environment variable (always available)
    let env_fn = lua.create_function(|_, name: String| Ok(std::env::var(&name).ok()))?;
    system.set("env", env_fn)?;

    // babu.system.platform - read-only string
