...
use babu_core::{commands, config, db, intent, JCommandsList, COMMANDS_LIST, DB};

fn print_help() {
    println!(
        "
--## Babu CLI - Testing Tool ##--
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Babu CLI v{}", config::APP_VERSION.unwrap_or("unknown"));
    ...
    COMMANDS_LIST
        .set(cmds)
        .expect("Failed to set commands list");
    ...
    if let Err(e) = babu_core::audio::init() {
